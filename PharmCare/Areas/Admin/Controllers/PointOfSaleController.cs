...
namespace PharmCare.Areas.Admin.Controllers
{
    ...
    public class PointOfSaleController : Controller
    {
        ...
        public async Task<ActionResult> GetReceipt(SalesDTO salesDTO)
        {
            try
            {
                if (salesDTO.ReceiptNo != null)
                {
                    var user = await userManager.FindByEmailAsync(User.Identity.Name);
                    ...
                    dt = await GetTransaction(salesDTO.ReceiptNo);

                    var getSale = await salesRepository.GetSaleByReceiptNo(salesDTO.ReceiptNo);
